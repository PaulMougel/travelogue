...
exports.register = function (server, options, next) {
    ...
    server.auth.scheme('passport', internals.implementation(settings));
            ...
            if (isExcludable) {
                ...
                if (sessionUser || sessionUser === 0) {
                    request._passport.instance.deserializeUser(sessionUser, function (err, user) {
                        if (err) {
                            return reply(Boom.internal('Failed to deserialize user', err));
                        }
                        if (!user) {
                ...
                next = function (err) {
                    if (err && err.isBoom) {
                        ...
                        if (settings.apiMode || options.apiMode) {
                            return err ? reply(Boom.unauthorized('Unauthorized'))
...
internals.allFailedFactory = function (request, reply, failures, options, callback) {
    ...
    return function allFailed() {
        ...
        return reply(Boom.unauthorized('Unauthorized', rchallenge || null));
    };
};
        ...
        var scheme = {
            authenticate: function (request, reply) {
                if (request.session._isAuthenticated()) {
                    return reply.continue({ credentials: request.session.user });
                }

                if (settings.apiMode || options.apiMode) {
                    return reply(new Boom.unauthorized('Unauthorized'));
                }
